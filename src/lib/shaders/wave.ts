...
void main() {
  ...
  // Create wave effect
  float wave = sin(noiseCoord.x * 10.0 + uTime) * 0.1;
  wave += sin(noiseCoord.y * 8.0 + uTime) * 0.1;
  
  float noise = snoise(vec3(noiseCoord.x + uTime, noiseCoord.y, uTime * 5.0));
  vec3 pos = vec3(position.x, position.y, position.z + wave + noise * 0.2);
